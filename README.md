...
Обработка геометок с фильтрацией:

```java
@BotLocation(
    actionName = "location-action",
    withinRadiusMeters = 500,
    centerLat = 53.9,
    centerLon = 27.5667,
...
* **`actionName`** — Уникальное имя действия, используемое для внутренней маршрутизации.
* **`withinRadiusMeters`** — Радиус (в метрах), в пределах которого координаты считаются допустимыми (при указанных `centerLat` и `centerLon`). `≤ 0` отключает проверку.
* **`centerLat`** — Широта центра области, от которой измеряется радиус.
* **`centerLon`** — Долгота центра области, от которой измеряется радиус.
* **`requireAccurateLocation`** — Требует точную геометку (игнорирует примерные координаты).
* **`minAccuracyMeters`** — Минимально допустимая точность координат (в метрах). `≤ 0` отключает проверку.
* **`maxAgeSeconds`** — Максимальное время "старения" геометки (в секундах). Старые сообщения игнорируются. `≤ 0` отключает проверку.
* **`requireExplicitLocation`** — Обрабатывает только геометки, отправленные напрямую (исключает пересланные и ответы).
* **`order`** — Приоритет обработчика. Меньшее значение — выше приоритет.
